...
const RUN_COMMANDS = new Set(['list', 'show', 'supersede', 'purge']);
...
Commands:
  ...
  runs [command]          Inspect or manage repository-local loop evidence
...
Run commands:
  ...
  runs supersede <id> [directory] --confirm
                          Start a linked v2 run and preserve legacy evidence
  runs purge [directory] --force
                          Remove package-owned local run evidence

Eval commands:
  eval validate           Validate the packaged 105-case corpus
...
Options:
  ...
  --confirm               Confirm explicit legacy-run supersession
...
export function parseArgs(argv) {
  ...
  if (result.command === 'runs') {
    ...
    if (['show', 'supersede'].includes(result.runCommand)) {
      result.runId = args.shift();
      if (!result.runId) throw new Error(`runs ${result.runCommand} requires a run ID`);
    }
  }
  ...
  while (args.length) {
    ...
    else if (arg === '--confirm' && result.command === 'runs' && result.runCommand === 'supersede') result.confirm = true;
